...
class NwsdbAnalytics extends StatefulWidget {
  const NwsdbAnalytics({Key? key}) : super(key: key);
...
class _NwsdbAnalyticsState extends State<NwsdbAnalytics> {
  ...
  void initState() {
    super.initState();
    _averageConsumption = averageConsumption('nwsdbBillsDatabase');
    _averageExpenditure = averageExpense('nwsdbBillsDatabase');
    _totalConsumption = totalConsumption('nwsdbBillsDatabase');
    _highestMonthlyConsumption = highestConsumption('nwsdbBillsDatabase');
    _highestMonthlyConsumptionMonth =
        highestConsumptionMonth('nwsdbBillsDatabase');
    _highestMonthlyExpenditure = highestExpense('nwsdbBillsDatabase');
    _lastBill = lastBill('nwsdbBillsDatabase');
    // _currentBalance = currentBalance('electricityBalance');
    void fetchCurrentBalance() async {
      _currentBalance = await currentBalance('waterBalance');
      setState(() {});
    }
        ...
        children: [
          ...
          Container(
            ...
            child: const Padding(
              ...
              child: BarGraph(billDatabase: 'nwsdbBillsDatabase'),
            ),
          ),
          const SizedBox(height: 24),
          _buildInfoCard(
